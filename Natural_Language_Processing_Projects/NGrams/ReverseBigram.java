...
public class ReverseBigram implements NgramInterface{
	...
	public void make( String filename ) throws IOException{
		...
		while( f.ready()){
			...
			//handle first case
			int tSize = tokens.length - 1;
			if( gram.containsKey(tokens[tSize]) == false ){
				gram.put( tokens[tSize], new HashMap<String,Integer>());
				columnCount.put( tokens[tSize], 0 );
			}
			String prev = tokens[tSize];
