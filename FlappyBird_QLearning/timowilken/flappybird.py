...
def main():
	...
	while not done:
		...
		#afterJump(bird,pp, pipe_collision , score)
		afterJump(bird,pp, done , score)
...
def jump(bird,pipe):
	...
	birdX = int(bird.x / quantization)
	birdY = int(bird.y / quantization)
	poleX = int(int(pipe.x) / quantization)
	poleHeight = int(pipe.PIECE_HEIGHT /quantization)
	
	x = int( (bird.x - pipe.x)/quantization )
	y = int( (bird.y - pipe.top_height_px)/quantization)
	
	jumpSA = (str(x)+","+str(y)+",1")
	notJumpSA = (str(x)+","+str(y)+",0")
	...
	if(Q[jumpSA] > mini or Q[notJumpSA] > mini ):
		rand = 2000
	if( rand < (ChanceRandomMove*10)/(numGames) ):
		rand = randint(0,3)
		...
		if( Q[jumpSA] > Q[notJumpSA] ):
			
			choice = jumpSA
		elif( Q[jumpSA] == Q[notJumpSA]):
			ran = randint(0,1)
			choice = notJumpSA
			if ran == 0:
				choice = jumpSA
...
def afterJump(bird,pipe,dead,score):
	global counter
	if counter == counter_max:
		counter = 0
	if  dead:
	...
	r = r - abs(y)
	Qsample = r + gamma * Q[choice]

	c = 10
	for ind in range( max(len(previousSA) - c+1,0), len(previousSA)):
		sa = previousSA[ind]
		alpha = float(1)/N[sa]
		Q[sa] = Q[sa] + alpha*(Qsample - Q[sa])/(c)
		c = c-1


def initQLearning(bird):
	global Q
	global N
	
	#x position of bird (constant)
	#y position of bird quantized
	#pole x position quantized
	#pole depth
	#dead or not
	
	
	xSpread = WIN_WIDTH / quantization + 5
	ySpread = WIN_HEIGHT / quantization + 5
	
	'''for birdY in range(-5,WIN_HEIGHT/quantizationY+5):
		for x in range(-1* xSpread, xSpread):
			for y in range(-1*ySpread, ySpread):
				for action in range(0,2):
					Q[ str(birdY)+","+str(x)+","+str(y)+","+str(action) ] = 0
					N[ str(birdY)+","+str(x)+","+str(y)+","+str(action) ] = 0'''
	for x in range(-1* xSpread, xSpread):
		for y in range(-1*ySpread, ySpread):
			for action in range(0,2):
				Q[ str(x)+","+str(y)+","+str(action) ] = 0
				N[ str(x)+","+str(y)+","+str(action) ] = 0
Q = {}
N  = {}
numGames = 0
