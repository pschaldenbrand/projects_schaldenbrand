...
public class Quadgram implements NgramInterface{
	...
	public void make( String filename ) throws IOException{
		...
		while( f.ready()){
			...
			if( tokens.length < 3 ){
				continue;	//only two words on this line
			}
			String second = tokens[0] + " " + tokens[1];
			if( gram.containsKey( second ) == false ){
				gram.put( second, new HashMap<String,Integer>() );
				columnCount.put( second , 0 );
			}
			else{	columnCount.put( second, columnCount.get( second ) + 1); }
			HashMap<String,Integer> secondCol = gram.get( second );
			if( secondCol.containsKey( tokens[2] ) == false ){
				secondCol.put( tokens[2] , 1 );
			}
			else{
				secondCol.put( tokens[2], secondCol.get( tokens[2]) + 1 );
			}
			
			String prev1 = tokens[0];
			String prev2 = tokens[1];
			String prev3 = tokens[2];
			
			for( int i = 3; i < tokens.length; i++ ){
				String next = tokens[i];
				String prevword = prev1 + " " + prev2 + " " + prev3;
	...
	public String perplexity( String line ){
		...
		curr = wordPerplex( prev1+" "+prev2, ar[2] );
		perplex = perplex.multiply( curr, mc );
		...
		for( int i = 3 ; i < ar.length ; i++ ){
			next = ar[i];
			curr = wordPerplex( prev1+" "+prev2 + " " + prev3, next );
			perplex = perplex.multiply( curr, mc );
			prev1 = prev2;
			prev2 = prev3;
			prev3 = next;
