...
public class ReverseTrigram implements NgramInterface{
	...
	public void make( String filename ) throws IOException{
		...
		while( f.ready()){
			...
			if( gram.containsKey(tokens[l]) == false ){
				gram.put( tokens[l], new HashMap<String,Integer>());
				columnCount.put( tokens[l], 0 );
			}
			else{ columnCount.put( tokens[l], columnCount.get(tokens[l])+1); }
			HashMap<String,Integer> firstCol = gram.get(tokens[l]);
			if( tokens.length < 2 ){ 
				continue; //only one word on this line
			}
			if( firstCol.containsKey( tokens[l-1] ) == false ){
				firstCol.put( tokens[l-1], 1);
			}
			else{
				firstCol.put( tokens[l-1], firstCol.get(tokens[l-1])+1 );
			}
			String prev1 = tokens[l];
			String prev2 = tokens[l-1];
	...
	public String perplexity( String line ){
		...
		String prev1 = ar[l];
		String prev2 = ar[l-1];
		BigDecimal curr = wordPerplex( prev1, ar[l-1] );
